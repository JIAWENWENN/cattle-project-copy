...
import { Plus, Save, ArrowLeft, Check, X, Search, ChevronDown, AlertTriangle } from 'lucide-vue-next';

defineOptions({
    layout: (h, page) => h(AppLayout, { title: 'Edit Calving Checklist', parent: 'Calving Checklist', parentUrl: '/calving-checklist' }, () => page)
});
...
const normalizeDateForInput = (value) => {
    ...
    // Keep plain date values exact (no timezone conversion).
    if (/^\d{4}-\d{2}-\d{2}$/.test(raw)) {
        return raw;
    }

    // For datetime values, convert to local calendar date to match table display.
...
const form = useForm({
    _method: 'PUT',
    id: props.checklistRecord.id,
    month_year: convertMonthYearToInputFormat(props.checklistRecord.month_year) || '2024-09',
    operating_unit: props.checklistRecord.operating_unit || props.checklistRecord.location || '',
    week: props.checklistRecord.week || '',
    calving_date: normalizeDateForInput(props.checklistRecord.calving_date),
    tag_no: props.checklistRecord.tag_no || '',
    sex: props.checklistRecord.sex || 'MC',
    colour: props.checklistRecord.colour || '',
    dam_tag_no: props.checklistRecord.dam_tag_no || '',
    dam_colour: props.checklistRecord.dam_colour || '',
    sire_tag_no: props.checklistRecord.sire_tag_no || '',
    sire_colour: props.checklistRecord.sire_colour || '',
    times_of_pregnancy: props.checklistRecord.times_of_pregnancy || '',
    location: props.checklistRecord.location || props.checklistRecord.operating_unit || '',
    location_block: matchLocationOption(props.allBlocks, props.checklistRecord.location_block),
    location_phase: matchLocationOption(props.allPhases, props.checklistRecord.location_phase),
    general_condition: props.checklistRecord.general_condition || 'Good',
    treatment_iodine: !!props.checklistRecord.treatment_iodine,
    treatment_woundsarex: !!props.checklistRecord.treatment_woundsarex,
    colostrum_feeding_24h: !!props.checklistRecord.colostrum_feeding_24h,
    mamumune: !!props.checklistRecord.mamumune,
    tagging_checklist_date: normalizeDateForInput(props.checklistRecord.tagging_checklist_date),
    lcc_running_number: props.checklistRecord.lcc_running_number || '',
    remarks: props.checklistRecord.remarks || ''
});
...
const linkedCattle = computed(() => findCattleByTag(props.checklistRecord.tag_no || form.tag_no));

// Searchable dropdown for re-linking cattle in edit
...
const selectedCattleLabel = computed(() => {
    const cattle = props.cattle.find(c => c.id === selectedCattleId.value);
    if (!cattle) return form.tag_no || '';
    return [cattle.tag_no, cattle.category, cattle.coat_colour].filter(Boolean).join(' - ');
});
...
const toTitleCase = (value) => {
    if (!value) return '';
    return String(value).toLowerCase().split(' ').filter(Boolean).map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
};
...
const resolveOperatingUnitFromCattle = (cattle) => {
    ...
    if (blockName && props.operatingUnitsWithStructure?.length) {
        ...
        if (estate?.name) return estate.name;
    }
...
const applyCattleToForm = (cattle) => {
    ...
    const calvingRec = getCalvingRecordForTag(cattle.tag_no);
    form.lcc_running_number = pickFirstNonEmpty(calvingRec?.lcc_running_number, cattle.lcc_running_number, form.lcc_running_number);
...
const hydrateFormFromLinkedCattle = () => {
    const cattle = linkedCattle.value;
    if (cattle) {
...
const closeDropdownOnClickOutside = (event) => {
    ...
    if (!dropdown && showCattleDropdown.value) showCattleDropdown.value = false;
};
...
const treatmentIodineWoundsarex = ref(!!props.checklistRecord.treatment_iodine || !!props.checklistRecord.treatment_woundsarex);
...
const submitForm = () => {
    ...
    form.post(`/calving-checklist/${props.checklistRecord.id}`, {
        onSuccess: () => {
    ...
    <div class="w-full max-w-5xl mx-auto">
        <div class="mb-8 flex justify-between items-center">
            <div class="flex items-center gap-4">
                <button @click="goBack" class="flex items-center gap-2 px-4 py-2 border border-gray-200 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
                    <ArrowLeft class="w-4 h-4" />
                    Back
                </button>
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Calving Checklist Record</h1>
                    <p class="text-sm text-gray-500 mt-1">Tag No.: {{ checklistRecord.tag_no || '-' }}</p>
                </div>
            </div>
        </div>
        ...
        <form @submit.prevent="submitForm">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div class="relative md:col-span-4">
                        ...
                        <div
                            ...
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-lg bg-white text-sm outline-none cursor-pointer flex items-center justify-between focus:ring-2 focus:ring-[#34554a]"
                        >
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" v-model="form.dam_tag_no" readonly class="w-full px-4 py-2.5 border border-gray-200 rounded-lg bg-gray-50 text-sm cursor-not-allowed" />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" v-model="form.dam_colour" readonly class="w-full px-4 py-2.5 border border-gray-200 rounded-lg bg-gray-50 text-sm cursor-not-allowed" />
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 mb-1">Sire Colour</label>
                        <input type="text" v-model="form.sire_colour" readonly class="w-full px-4 py-2.5 border border-gray-200 rounded-lg bg-gray-50 text-sm cursor-not-allowed" />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select v-model="form.colour" disabled class="w-full px-4 py-2.5 border border-gray-200 rounded-lg bg-gray-50 text-sm cursor-not-allowed">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" :value="form.location_block" readonly class="w-full px-4 py-2.5 border border-gray-200 rounded-lg bg-gray-50 text-sm cursor-not-allowed" />
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 mb-1">Phase</label>
                        <input type="text" :value="form.location_phase" readonly class="w-full px-4 py-2.5 border border-gray-200 rounded-lg bg-gray-50 text-sm cursor-not-allowed" />
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <input
                            type="text"
                            v-model="form.lcc_running_number"
                            :disabled="hasAutoFilledFields"
                            placeholder="e.g., 3179"
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-[#34554a]"
                            :class="hasAutoFilledFields ? 'bg-gray-50 cursor-not-allowed' : 'bg-white'"
                        />
                    </div>
                    <div>
            ...
            <div class="flex justify-end">
                <button type="submit" class="flex items-center gap-2 px-6 py-3 bg-[#34554a] text-white rounded-lg text-sm font-medium hover:bg-[#2a443b] shadow-sm transition-colors" :disabled="form.processing">
                    <Save class="w-4 h-4" />
