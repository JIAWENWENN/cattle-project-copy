...
defineOptions({
    layout: (h, page) => {
        const type = String(page.props.document?.type || '').toLowerCase();
        const isReceival = type === 'receival';
        const isCtv = type === 'ctv';
        const title = isReceival ? 'Receival Workflow' : isCtv ? 'CTV Workflow' : 'CTV Workflow';
        const parentUrl = isReceival ? '/transfer/receival' : isCtv ? '/transfer/ctv' : '/transfer/siv';
        return h(AppLayout, { title, parent: 'Transfer', parentUrl }, () => page);
    }
});

const page = usePage();
const document = computed(() => page.props.document || {});
const isReceival = computed(() => String(document.value?.type || '').toLowerCase() === 'receival');
const isCtv = computed(() => String(document.value?.type || '').toLowerCase() === 'ctv');
...
const permissionModule = computed(() => {
    if (isReceival.value) return 'Transfer Receival';
    if (isSiv.value) return 'Transfer SIV';
    return 'Transfer CTV';
});

const modulePermissions = computed(() => {
    const perms = page.props.auth?.permissions?.[permissionModule.value];
...
const isSiv = computed(() => !isCtv.value && !isReceival.value);

const computedParentUrl = computed(() => {
    if (isReceival.value) return '/transfer/receival';
    if (isCtv.value) return '/transfer/ctv';
    return '/transfer/siv';
});

const roleByStep = computed(() => isReceival.value
    ? ['livestock', 'assistant_manager', 'security']
    : ['livestock', 'livestock manager', 'driver', 'assistant_manager', 'security', 'livestock', 'supervisor', 'security']
);
const assignmentKeyByStep = computed(() => isReceival.value
    ? ['issued_by_user_ids', 'witnessed_transit_by_user_ids', 'verified_transit_by_user_ids']
    : ['issued_by_user_ids', 'approved_by_user_ids', 'transported_by_user_ids', 'witnessed_transit_by_user_ids', 'verified_transit_by_user_ids', 'witnessed_receive_by_user_ids', 'received_by_user_ids', 'completed_by_user_ids']
);

const normalizeRole = (role) => {
    const r = String(role || '').toLowerCase().replace(/[._-]/g, ' ').replace(/\s+/g, ' ').trim();
    if (['manager', 'livestock manager', 'act livestock manager', 'act. livestock manager'].includes(r)) return 'livestock manager';
    if (['senior assistant manager', 'sr assistant manager', 'assistant manager'].includes(r)) return 'assistant_manager';
    if (['gate house security', 'sr assistant security', 'senior assistant security'].includes(r)) return 'security';
    return r;
};

const legacyKeyByStep = computed(() => isReceival.value
    ? ['issued_by_user_id', 'witnessed_transit_by_user_id', 'verified_transit_by_user_id']
    : ['issued_by_user_id', 'approved_by_user_id', 'transported_by_user_id', 'witnessed_transit_by_user_id', 'verified_transit_by_user_id', 'witnessed_receive_by_user_id', 'received_by_user_id', 'completed_by_user_id']
);

const getAssignedIdsForStep = (stepIndex) => {
    const key = assignmentKeyByStep.value[stepIndex];
    const legacyKey = legacyKeyByStep.value[stepIndex];
...
const getWaitingMessage = (stepIndex) => {
    if (!canUserHandleStep(stepIndex) || canUploadStep(stepIndex)) return '';
    if (isCompleted.value) return 'Workflow completed';
    const current = Number(document.value?.endorsement_step ?? 0);
    if (stepIndex > current) return 'Waiting for previous steps';
    return 'Waiting';
};

const workflowSteps = computed(() => {
    const defaults = isReceival.value
        ? [
            { id: 'issued', label: 'Prepared By', role_name: '' },
            { id: 'witness_transit', label: 'Witness By', role_name: '' },
            { id: 'verified_transit', label: 'Verified By', role_name: '' },
        ]
        : [
            { id: 'issued', label: 'Issued By (Transferor Estate)', role_name: 'Position' },
            { id: 'approved', label: 'Approved By (Transferor Estate)', role_name: 'Livestock Manager/OIC' },
            { id: 'transported', label: 'Transported By (Transferor Estate)', role_name: 'Driver' },
            { id: 'witness_transit', label: 'Witness By (Transferor Estate)', role_name: 'Estate Personal' },
            { id: 'verified_transit', label: 'Verified By (Transferor Estate)', role_name: 'Gate House Security' },
            { id: 'witness_receive', label: 'Witness By (Receiving Estate)', role_name: 'Estate Personal' },
            { id: 'received', label: 'Received By (Receiving Estate)', role_name: 'Livestock Supervisor/Assistant' },
            { id: 'completed', label: 'Verified Completion By (Receiving Estate)', role_name: 'Gate House Security' },
        ];

    return defaults.map((step, index) => ({
        ...step,
        role: roleByStep.value[index],
    }));
});

const transferorSteps = computed(() => {
    if (isReceival.value) return workflowSteps.value;
    return workflowSteps.value.slice(0, 5);
});
const receivingSteps = computed(() => {
    if (isReceival.value) return [];
    return workflowSteps.value.slice(5);
});

const getStatusStepIndex = (status) => {
    const steps = isReceival.value || isCtv.value
        ? ['pending', 'issued', 'witness_transit', 'verified_transit', 'completed']
        : ['pending', 'issued', 'approved', 'transported', 'witness_transit', 'verified_transit', 'witness_receive', 'received', 'completed'];
...
const canUploadStep = (stepIndex) => {
    ...
    const current = Number(document.value?.endorsement_step ?? 0);
    const lastStepIndex = workflowSteps.value.length - 1;

    if (stepIndex === lastStepIndex) return stepIndex <= current;
    if (current === stepIndex) return true;
...
const goBack = () => router.visit(computedParentUrl.value);
</script>
    ...
    <div class="w-full">
        <div class="mb-6 flex justify-between items-center">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 tracking-tight">{{ isReceival ? 'Receival Workflow' : isCtv ? 'CTV Workflow' : 'CTV Workflow' }} - {{ document.document_no || '-' }}</h1>
                    <p class="text-sm text-gray-500 mt-1">Workflow design and logic aligned with calving pattern</p>
                </div>
            </div>
        </div>
        ...
        <WorkflowEndorsementCards
            ...
            :user-name="userName"
            :grid-class="isReceival ? 'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4' : 'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4'"
