...
defineOptions({
    layout: (h, page) => h(AppLayout, { title: 'Edit Receival', parent: 'Transfer', parentUrl: '/transfer/receival' }, () => page)
});
...
const form = useForm({
    form_document_no: doc.value.form_document_no || '',
    revision_no: doc.value.revision_no || '',
    ...
    vehicle_no: doc.value.vehicle_no || '',
    driver_name: doc.value.driver_name || '',
    driver_tel: doc.value.driver_tel || '',
    driver_ic: doc.value.driver_ic || '',
    customer_name: doc.value.customer_name || '',
...
if (!form.livestock.length) {
    form.livestock.push({ row_key: Date.now(), tag_no: '', category: 'WB', colour: '', weight: null, condition_good: true, condition_not_good: false, remarks: '', purpose: '', yard: '' });
}
...
const addLivestock = () => {
    form.livestock.push({ row_key: Date.now() + Math.random(), tag_no: '', category: 'WB', colour: '', weight: null, condition_good: true, condition_not_good: false, remarks: '', purpose: '', yard: '' });
};
...
const selectTagForRow = (row, cattle) => {
    ...
    const alreadyExists = form.livestock.some((item) => item.tag_no === tagNo && item.row_key !== row.row_key);
    if (alreadyExists) {
    ...
    <div class="w-full max-w-7xl mx-auto">
        <div class="mb-8 flex justify-between items-center">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Edit receival document</h1>
                    <p class="text-sm text-gray-500 mt-1">Update receival details and livestock list</p>
                </div>
            </div>
        ...
        <form @submit.prevent="submitForm">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
                <div class="px-6 py-4 bg-[#34554a] border-b border-gray-100">
                    <h3 class="text-sm font-bold text-white">Receival scope</h3>
                </div>
                <div class="p-6 grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-6">
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 mb-1">Document no.</label>
                        <input v-model="form.form_document_no" type="text" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg bg-white text-sm outline-none focus:ring-2 focus:ring-[#34554a]">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 mb-1">Revision no.</label>
                        <input v-model="form.revision_no" type="text" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg bg-white text-sm outline-none focus:ring-2 focus:ring-[#34554a]">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 mb-1">From location *</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 mb-1">Driver tel</label>
                        <input v-model="form.driver_tel" type="text" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg bg-white text-sm outline-none focus:ring-2 focus:ring-[#34554a]">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 mb-1">Driver IC</label>
                        <input v-model="form.driver_ic" type="text" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg bg-white text-sm outline-none focus:ring-2 focus:ring-[#34554a]">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 mb-1">Customer name</label>
                        <input v-model="form.customer_name" type="text" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg bg-white text-sm outline-none focus:ring-2 focus:ring-[#34554a]">
                    </div>
                </div>
            </div>
                    ...
                    <table class="w-full text-left whitespace-nowrap min-w-[1100px]">
                        ...
                        <tbody class="divide-y text-sm text-gray-700">
                            <tr v-for="(item, index) in form.livestock" :key="item.row_key" class="hover:bg-gray-50">
                                    ...
                                    <div class="relative">
                                        ...
                                        >
                                            <span :class="item.tag_no ? 'text-gray-900' : 'text-gray-400'" class="text-sm">
                                                {{ item.tag_no || 'Search and select tag no...' }}
                                            </span>
                                            ...
                                            <div class="p-3 border-b border-gray-100 bg-gray-50">
                                                <div class="relative">
                                                    ...
                                                    <input
                                                        ...
                                                        type="text"
                                                        placeholder="Search by tag no, ID, category, or colour..."
                                ...
                                <td class="p-3">
                                    <input v-model="item.colour" class="w-full px-3 py-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-[#34554a]" :disabled="!!item.tag_no" :class="{'bg-gray-100 cursor-not-allowed': !!item.tag_no}">
                                </td>
                                ...
                                <td class="p-3">
                                    <div class="flex items-center gap-3">
                                        <label class="inline-flex items-center gap-1"><input type="checkbox" v-model="item.condition_good"> Normal</label>
                                        <label class="inline-flex items-center gap-1"><input type="checkbox" v-model="item.condition_not_good"> Abnormal</label>
                                    </div>
                                </td>
                                <td class="p-3"><input v-model="item.yard" class="w-full px-3 py-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-[#34554a]"></td>
