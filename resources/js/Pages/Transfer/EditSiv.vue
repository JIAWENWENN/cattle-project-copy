...
defineOptions({
    layout: (h, page) => h(AppLayout, { title: 'Edit SIV', parent: 'Transfer', parentUrl: '/transfer/siv' }, () => page)
});
...
const form = useForm({
    type: 'SIV',
    from_location: document.value.from_location || '',
    to_location: document.value.to_location || '',
    date: document.value.date ? String(document.value.date).slice(0, 10) : new Date().toISOString().split('T')[0],
    time: document.value.time ? String(document.value.time).slice(0, 5) : new Date().toTimeString().slice(0, 5),
    vehicle_no: document.value.vehicle_no || '',
    driver_name: document.value.driver_name || '',
    driver_tel: document.value.driver_tel || '',
    driver_ic: document.value.driver_ic || '',
    address: document.value.address || '',
    customer_name: document.value.customer_name || '',
    siv_no: document.value.siv_no || '',
    receipt_no: document.value.receipt_no || '',
    livestock: (document.value.livestock || []).map((row) => ({
        row_key: Date.now() + Math.random(),
        tag_no: row.tag_no || '',
        category: row.category || 'WB',
        colour: row.colour || '',
...
const submitForm = () => {
    ...
    form.put(`/transfer/siv/${document.value.id}`, {
        onSuccess: () => {
            alert('SIV updated successfully');
        },
        onError: (errors) => {
            const message = Object.values(errors || {}).flat().join('\n') || 'Failed to update SIV';
            alert(message);
        }
    });
};
    ...
    <div class="w-full max-w-6xl mx-auto">
        <div class="mb-8 flex justify-between items-center">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Edit sales issue voucher (SIV)</h1>
