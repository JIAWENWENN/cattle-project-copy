...
defineOptions({
    layout: (h, page) => h(AppLayout, { title: 'Receival History', parent: 'Transfer', parentUrl: '/transfer/receival' }, () => page)
});

const page = usePage();
const documents = computed(() => page.props.documents?.data || []);

const receivalPermissions = computed(() => {
    const perms = page.props.auth?.permissions?.['Transfer Receival'];
    return Array.isArray(perms) ? perms : ['no-access'];
});
const hasReceivalPermission = (action) => {
    if (String(page.props.auth?.user?.role || '').toLowerCase() === 'admin') return true;
    const perms = receivalPermissions.value;
    return perms.includes('full') || perms.includes(action);
};
const canViewReceival = computed(() => hasReceivalPermission('view'));
const canCreateReceival = computed(() => hasReceivalPermission('create'));
const canEditReceival = computed(() => hasReceivalPermission('edit'));
const canDeleteReceival = computed(() => hasReceivalPermission('delete'));
...
const bulkDelete = () => {
    if (!canDeleteReceival.value || selectedRecords.value.length === 0) return;
...
const conditionLabel = (item) => {
    if (item?.condition_good) return 'Normal';
    if (item?.condition_not_good) return 'Abnormal';
    return '-';
};
...
const clearFilters = () => {
    ...
    router.get('/transfer/receival', {}, { preserveState: true });
};

const goToCreate = () => {
    if (!canCreateReceival.value) return;
    router.visit('/transfer/create/receival');
};
const editRecord = (doc) => {
    if (!canEditRecord(doc)) return;
    router.visit(`/transfer/receival/${doc.id}/edit`);
};
const viewWorkflow = (doc) => {
    if (!canWorkflowRecord(doc)) return;
    router.visit(`/transfer/receival/${doc.id}/workflow`);
};
...
const openViewPopup = (doc) => {
    if (!canViewReceival.value) return;
...
const confirmDelete = () => {
    ...
    router.delete(`/transfer/receival/${deleteTarget.value.id}`, {
...
const canWorkflowRecord = (record) => {
    if (!record || !canViewReceival.value) return false;
    return true;
};
...
const canEditRecord = (record) => {
    if (!record || !canEditReceival.value) return false;
    return !isRecordLocked(record);
};

const canDeleteRecord = (record) => {
    if (!record || !canDeleteReceival.value) return false;
    return !isRecordLocked(record);
};

const getWorkflowDisabledReason = (record) => {
    if (!record) return 'Workflow unavailable';
    if (!canViewReceival.value) return 'You do not have view permission';
...
const getEditDisabledReason = (record) => {
    ...
    if (!canEditReceival.value) return 'You do not have edit permission';
...
const getDeleteDisabledReason = (record) => {
    ...
    if (!canDeleteReceival.value) return 'You do not have delete permission';
...
<template>
    <div class="w-full">
        <div class="mb-8 flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Receival history</h1>
                <p class="text-sm text-gray-500 mt-1">Receival records from suppliers</p>
            </div>
            <div class="flex items-center gap-3">
                <button
                    v-if="canDeleteReceival"
                ...
                <button v-if="canCreateReceival" @click="goToCreate" class="flex items-center gap-2 bg-[#34554a] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-[#2a443b] shadow-sm transition-colors">
                    <Plus class="w-4 h-4" />
                    Create Receival
                </button>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <table class="w-full text-left whitespace-nowrap">
                    <thead>
                        <tr class="bg-[#34554a] text-white text-sm">
                            <th v-if="canDeleteReceival && showBulkActions" class="p-4 font-semibold w-12 text-center">
                                <input
                                    type="checkbox"
                                    :checked="allSelected"
                            ...
                            <th class="p-4 font-semibold w-12">{{ selectedRecords.length > 0 ? '#' : '#' }}</th>
                    ...
                    <tbody class="divide-y text-sm text-gray-700">
                        <tr v-for="(doc, index) in paginatedDocs" :key="doc.id"
                            ...
                            <td v-if="canDeleteReceival && showBulkActions" class="p-4 text-center">
                                <input
                                    type="checkbox"
                            ...
                            <td class="p-4 text-right flex justify-end gap-2">
                                <span v-if="canEditReceival" class="inline-flex" :title="getEditDisabledReason(doc)">
                                    <button
                                ...
                                <span v-if="canDeleteReceival" class="inline-flex" :title="getDeleteDisabledReason(doc)">
                                    <button
                        ...
                        <tr v-if="filteredDocs.length === 0">
                            <td :colspan="canDeleteReceival && showBulkActions ? 8 : 7" class="p-8 text-center text-gray-400 italic">No Receival records found</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="flex flex-col md:flex-row justify-between items-center p-4 border-t border-gray-100 bg-gray-50">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button v-for="pageNumber in pageNumbers" :key="`receival-page-${pageNumber}`" @click="pageNumber !== '...' && goToPage(pageNumber)" :class="[pageNumber === currentPage ? 'bg-[#34554a] text-white' : 'text-gray-600 hover:bg-white', pageNumber === '...' ? 'cursor-default' : '']" class="w-8 h-8 flex items-center justify-center rounded-lg text-sm font-bold transition-colors">
                        {{ pageNumber }}
                    </button>
                ...
                <div class="bg-white rounded-xl shadow-xl w-full max-w-7xl mx-auto overflow-hidden border border-gray-100 max-h-[90vh] flex flex-col" @click.stop>
                    <div class="flex justify-between items-center p-6 border-b border-gray-100 bg-gray-50 sticky top-0">
                        <h3 class="text-lg font-bold text-gray-900">
                            Receival Record Details - {{ selectedDoc.document_no || '-' }}
                        </h3>
                        <button
                            type="button"
                            @click="closeModal"
                    ...
                    <div class="p-6 overflow-y-auto flex-1">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-5">
                                <div class="bg-gray-50 rounded-xl p-4">
                                    ...
                                    <div class="space-y-2 text-sm">
                                        ...
                                        <div class="flex justify-between py-1.5 border-b border-gray-200">
                                            ...
                                            <span class="font-semibold text-gray-900">{{ selectedDoc.form_document_no || '-' }}</span>
                                        </div>
                                        <div class="flex justify-between py-1.5 border-b border-gray-200">
                                            <span class="text-gray-500">Revision No.</span>
                                            <span class="font-semibold text-gray-900">{{ selectedDoc.revision_no || '-' }}</span>
                                        </div>
                                        <div class="flex justify-between py-1.5 border-b border-gray-200">
                                ...
                                <div class="bg-gray-50 rounded-xl p-4">
                                    <h4 class="text-sm font-bold text-gray-800 mb-3">Receival Route</h4>
                                    <div class="space-y-2 text-sm">
                                        <div class="flex justify-between py-1.5 border-b border-gray-200">
                            ...
                            <div class="space-y-5">
                                <div class="bg-gray-50 rounded-xl p-4">
                                    <h4 class="text-sm font-bold text-gray-800 mb-3">Transport</h4>
                                    <div class="space-y-2 text-sm">
                                        <div class="flex justify-between py-1.5 border-b border-gray-200">
                                ...
                                <div class="bg-gray-50 rounded-xl p-4">
                                    ...
                                    <div class="space-y-2 text-sm">
                                        <div class="flex justify-between py-1.5 border-b border-gray-200">
                                            ...
                                            <span class="font-semibold text-gray-900">{{ selectedDoc.type || 'Receival' }}</span>
                                        </div>
                                        <div class="flex justify-between py-1.5 border-b border-gray-200">
                                        ...
                                        <div class="flex justify-between py-1.5">
                                            ...
                                            <span class="font-semibold text-gray-900">{{ selectedDoc.creator?.name || '-' }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <table class="w-full text-left whitespace-nowrap text-sm min-w-[1000px]">
                                    <thead>
                                        <tr class="bg-[#34554a] text-white">
                                            ...
                                            <th class="p-3 font-semibold">Receival Weight (KG)</th>
                                            <th class="p-3 font-semibold">Condition</th>
                                            <th class="p-3 font-semibold">Yard</th>
                                    ...
                                    <tbody class="divide-y text-gray-700">
                                        <tr v-for="(item, index) in selectedDoc.livestock || []" :key="`${item.id || item.tag_no}-${index}`" class="hover:bg-gray-50">
                                            ...
                                            <td class="p-3">{{ conditionLabel(item) }}</td>
                                            <td class="p-3">{{ item.yard || '-' }}</td>
        ...
        <DeleteConfirmationModal
            ...
            title="Delete Receival Record"
            message="Are you sure you want to delete this Receival record"
        ...
        <DeleteConfirmationModal
            ...
            title="Delete Receival Records"
            message="Are you sure you want to delete the selected Receival records"
